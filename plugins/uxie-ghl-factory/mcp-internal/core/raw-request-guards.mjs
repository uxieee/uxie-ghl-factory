...
const RULES = [
  {
    ...
    refuses: (body) => !(isPlainObject(body) && Array.isArray(body.statusIds) && body.statusIds.length > 0),
  ...
  },
  {
    ...
    refuses: (body) => body === undefined || body === null || (isPlainObject(body) && Object.keys(body).length === 0),
  ...
  },
  {
    ...
    refuses: (body) => isPlainObject(body) && body.status === 'published',
  ...
  },
  {
    ...
    refuses: (body) => !(isPlainObject(body) && Object.hasOwn(body, 'permission')),
...
export function refuseRawRequest({ method, path, body }) {
  ...
  for (const r of RULES) {
    if (r.method === method && r.path.test(p) && r.refuses(body)) return { rule: r.rule, message: r.message, hint: r.hint };
  }
  return null;
}
