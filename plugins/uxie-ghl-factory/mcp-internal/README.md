...
| `edit_workflow` | read-only preview; writes require `confirm: true` and never publish; runs the build path's validation ladder pre-write — workflow + graph-context rules, GHL's asset-reference validator (`ignoreAssetErrors` hatch, stamped `phase: pre-write` and re-run over the persisted document after the write), the measured field caps (`allowOverCap` hatch), the custom-code sandbox on touched `custom_code` steps (`strictCustomCode`/`skipCustomCodeTest`), account-readiness signals, and a persisted required-field check in the round-trip verify. OP KEYS ARE STRICT — an unknown key refuses the call by name. Trigger writes are verified against what the CALLER asked for and against the server's own `date_updated` stamp, never against the engine's intent; a patch that changes nothing is a NOOP, not a write |
| `repair_workflow` | full-document replacement of `workflowData.templates` with EVERY edit guard (association, required fields, dangling refs/parentKeys, goto loops, dead branches, workflow rules, merge tags) AND the build path's pre-write ladder over the steps the repair changes (graph-context rules, action schema, asset validator with `ignoreAssetErrors`, custom-code sandbox with `strictCustomCode`/`skipCustomCodeTest`, readiness, persisted required fields + opportunity intent), then the plain PUT and a round-trip verify — the sanctioned replacement for a hand-rolled PUT; `expectedVersion` refuses a stale read |
| `update_convai_agent` | READ-MERGE-WRITE for an existing Conversation AI agent: `GET` the record, overlay the spec, apply the builder's own bot-type cleanup, `PUT` the WHOLE record, re-read, and diff every field the update did not set — a partial PUT resets omitted agent-level booleans, so a partial is never sent |
...
| `rename_workflow` | read-only preview of old → new names; renaming requires `confirm: true`, uses GHL's dedicated rename route (no step re-validation), refuses folder ids and blank names before sending, verifies each name by read-back |
...
| `raw_request` | GET escape hatch; refuses five call shapes with no legitimate version (measured to do silent damage or nothing); other non-GET methods require `confirm: true`, whose preview carries the route's measured `trap` note when one is known, and return partial-progress evidence. Separately (not one of the five — payload-scoped, not path-scoped, and no confirm hatch), any write whose body carries the scrubber's redaction placeholder is refused, naming the offending step(s); the same guard also covers `edit_workflow` and `repair_workflow`'s whole-document writes — see PROPOSAL-redacted-payload-guard.md |
