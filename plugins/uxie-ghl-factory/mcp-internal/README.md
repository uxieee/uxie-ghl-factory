...
| `edit_workflow` | read-only preview; writes require `confirm: true` and never publish; runs the build path's validation ladder pre-write — workflow + graph-context rules, GHL's asset-reference validator (`ignoreAssetErrors` hatch), the custom-code sandbox on touched `custom_code` steps (`strictCustomCode`/`skipCustomCodeTest`), account-readiness signals, and a persisted required-field check in the round-trip verify |
