...
// The manifest gate that used to live here is gone with the second server (2026-09-16).
// It compared these descriptors against a committed audit-capability-manifest.json, an
// artefact only the read-only audit profile consumed. The descriptors themselves did NOT
// go: core/audit-gateway.mjs still enforces every one of them on the MAIN server's
// receipt-gated composites, and the tests above are what hold them to it.
