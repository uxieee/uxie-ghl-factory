...
| `type` | What it does | Key attributes | Trap |
|---|---|---|---|
| `custom_webhook` | Arbitrary HTTP call, any method/auth | `url`, `method`, `body.rawData` (string template), `authorization` | **A 5xx is retried and HOLDS the run** (+5, +10, +15, +20 min… — later steps wait); **a 4xx fails and the run carries on at once**; a 200 with an error in its body counts as success — branch on `{{custom_webhook.N.status}}` / `.response.<path>` (live-proven 2026-09-25). `isPremiumAction: true` — requires the location's premium gate. Needs `stepIndex` + `advanceCanvasMeta` per `create-ghl-workflow/references/step-shapes.md`. |
