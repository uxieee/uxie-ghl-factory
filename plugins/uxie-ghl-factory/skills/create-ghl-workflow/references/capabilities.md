...
### Airtable
- ▫ `airtable_new_record_created` (UNKNOWN — you must author it) — filters: Base (`base_id`), Table (`table_id`)
- ▫ `airtable_record_updated` (UNKNOWN — you must author it) — filters: Base (`base_id`), Table (`table_id`)

### Apify
- ▫ `apify_actor_run_finished` (UNKNOWN — you must author it) — filters: Actor (`actor_id`), Status (`eventType`)
- ▫ `apify_task_run_finished` (UNKNOWN — you must author it) — filters: Task (`task_id`), Status (`eventType`)

### Asana
- ▫ `asana_it_asana_attachment_added_to_task` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`)
- ▫ `asana_it_asana_comment_on_task` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`), Task (`target.gid`), Comment Text (`comment_text`), Commenter Name (`created_by.name`)
- ▫ `asana_it_asana_new_subtask` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`), Task (`parent.gid`)
- ▫ `asana_it_asana_project_created` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`)
- ▫ `asana_it_asana_tag_added_to_task` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`), Tags (`tag_gid`)
- ▫ `asana_it_asana_task_created` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`)
- ▫ `asana_it_asana_task_deleted` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`)
- ▫ `asana_it_asana_task_moved_to_section` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`), Sections (`section_gids`)
- ▫ `asana_it_asana_task_updated` (UNKNOWN — you must author it) — filters: Workspace (`workspace_id`), Project (`project_id`), Status (`completed`), Fields Changed (`field`)

### BaseCamp
- ▫ `basecamp_new_activity` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`), Types (`activity_type`), Events (`events`)
- ▫ `basecamp_new_comment_added` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`)
- ▫ `basecamp_new_document` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`), Folder (`parent_id`)
- ▫ `basecamp_new_message_posted` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`)
- ▫ `basecamp_new_todo_created` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`), Todo Set (`to_do_set`), Todo List (`todolist_id`)
- ▫ `basecamp_new_todo_list` (UNKNOWN — you must author it) — filters: Account (`account_id`), Project (`project_id`), Todo Set (`parent_id`)
- ▫ `basecamp_project_created` (UNKNOWN — you must author it) — filters: Account (`account_id`), Name (`name`)

### Browse AI
- ▫ `lc_browse_ai_new_task` (UNKNOWN — you must author it) — filters: Robot (`robot_id`), Event Type (`eventType`)

### Cal.com
- ▫ `lc_cal_com_booking_cancelled` (UNKNOWN — you must author it)
- ▫ `lc_cal_com_booking_created` (UNKNOWN — you must author it)
- ▫ `lc_cal_com_booking_rescheduled` (UNKNOWN — you must author it)
- ▫ `lc_cal_com_meeting_ended` (UNKNOWN — you must author it)
- ▫ `lc_cal_com_ooo_created` (UNKNOWN — you must author it)
- ▫ `lc_cal_com_recording_ready` (UNKNOWN — you must author it)

### Calendly
- ▫ `ankit_test_invitee_created` (UNKNOWN — you must author it) — filters: Webhook Subscription Scope (`scope`)
- ▫ `calendly_event_cancelled` (UNKNOWN — you must author it) — filters: Organization (`organization`)
- ▫ `calendly_invitee_canceled` (UNKNOWN — you must author it) — filters: Webhook Subscription Scope (`scope`)
- ▫ `calendly_invitee_created` (UNKNOWN — you must author it) — filters: Webhook Subscription Scope (`scope`)
- ▫ `calendly_invitee_no_show_created` (UNKNOWN — you must author it) — filters: Webhook Subscription Scope (`scope`)
- ▫ `lc_calendly_new_routing_form_submission` (UNKNOWN — you must author it)

### ClickUp
- ▫ `clickup_comment_created` (UNKNOWN — you must author it) — filters: Workspace (`team_id`), Space (`space_id`), Folders (`folder_id`), List (`list_id`), Task (`task_id`)
- ▫ `clickup_new_folder` (UNKNOWN — you must author it) — filters: Workspace (`team_id`), Space (`space_id`)
- ▫ `clickup_new_list` (UNKNOWN — you must author it) — filters: WorkSpace (`team_id`), Space (`space_id`), Folders (`folder_id`)
- ▫ `clickup_new_task` (UNKNOWN — you must author it) — filters: Workspace (`team_id`), Space (`space_id`), Folders (`folder_id`), List (`list_id`)
- ▫ `clickup_new_time_entry` (UNKNOWN — you must author it) — filters: Workspace (`team_id`), Space (`space_id`), Folders (`folder_id`), List (`list_id`), Task (`task_id`)
- ▫ `clickup_task_updated` (UNKNOWN — you must author it) — filters: WorkSpace (`team_id`), Space (`space_id`), Folders (`folder_id`), List (`list_id`), Status (`status`), Priority (`priority`)
- ▫ `lc_cu_task_changes_internal` (UNKNOWN — you must author it) — filters: WorkSpace (`team_id`), Space (`space_id`), Folders (`folder_id`), List (`list_id`), Status (`status`), Priority (`priority`)
...
### Fathom
- ▫ `lc_fathom_new_recording` (UNKNOWN — you must author it) — filters: Triggers on (`triggers_on`)

### Google Ads
- ▫ `google_lead_form_submitted` (internal) — filters: Ad Account Is (`adAccountId`)

### Google Contacts
- ▫ `google_contacts_contact_created` (UNKNOWN — you must author it) — filters: Name (`name`), First Name (`firstName`), Last Name (`lastName`)
- ▫ `google_contacts_new_group` (UNKNOWN — you must author it) — filters: Name (`name`), Group Type (`groupType`)

### Google Forms
- ▫ `lc_gforms_new_updated_response` (UNKNOWN — you must author it) — filters: Form (`form_id`)

### Housecall Pro
- ▫ `lc_housecall_pro_customer_created` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_estimate_created` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_estimate_finished` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_estimate_scheduled` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_job_canceled` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_job_created` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_job_finished` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_job_scheduled` (UNKNOWN — you must author it)
- ▫ `lc_housecall_pro_lead_created` (UNKNOWN — you must author it)

### HubSpot
- ▫ `lc_hubspot_company_recently_created_or_updated` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_contact_created` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_contact_recently_created_or_updated` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_crm_object_recently_created` (UNKNOWN — you must author it) — filters: Object Type (`object_type`)
- ▫ `lc_hubspot_crm_object_recently_created_or_updated` (UNKNOWN — you must author it) — filters: Object Type (`object_type`)
- ▫ `lc_hubspot_custom_object_recently_created` (UNKNOWN — you must author it) — filters: Custom Object Type (`object_type`)
- ▫ `lc_hubspot_custom_object_recently_created_or_updated` (UNKNOWN — you must author it) — filters: Custom Object Type (`object_type`)
- ▫ `lc_hubspot_deal_recently_created_or_updated` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_new_company` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_new_deal` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_new_engagement` (UNKNOWN — you must author it) — filters: Engagement Type (`engagement_type`)
- ▫ `lc_hubspot_new_product` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_new_ticket` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_product_recently_created_or_updated` (UNKNOWN — you must author it)
- ▫ `lc_hubspot_ticket_recently_created_or_updated` (UNKNOWN — you must author it)

### Jira
- ▫ `lc_jira_issue_created` (UNKNOWN — you must author it) — filters: Cloud Site (`cloud_site`), Project (`project_id`), Issue Type (`issue_type_id`), Assignee (`assignee_id`), Status (`status_id`)
- ▫ `lc_jira_issue_update` (UNKNOWN — you must author it) — filters: Cloud Site (`cloud_site`), Project (`project_id`), Issue Type (`issue_type_id`), Assignee (`assignee_id`), Status (`status_id`)

### Klaviyo
- ▫ `lc_klaviyo_new_event` (UNKNOWN — you must author it) — filters: Metric (`metric_id`)
- ▫ `lc_klaviyo_new_profile_created` (UNKNOWN — you must author it)
- ▫ `lc_klaviyo_profile_added_to_list` (UNKNOWN — you must author it) — filters: List (`list_id`)
- ▫ `lc_klaviyo_profile_added_to_segment` (UNKNOWN — you must author it) — filters: Segment (`segment_id`)

### Linear
- ▫ `lc_linear_new_customer` (UNKNOWN — you must author it) — filters: Team (`team_id`)
- ▫ `lc_linear_new_customer_need` (UNKNOWN — you must author it) — filters: Team (`team_id`), Customer (`customer.id`)
- ▫ `lc_linear_new_document_comment` (UNKNOWN — you must author it) — filters: Team (`team_id`), Creator (`user.id`)
- ▫ `lc_linear_new_initiative_update` (UNKNOWN — you must author it) — filters: Team (`team_id`), Status (`health`), Creator (`user.id`)
- ▫ `lc_linear_new_issue` (UNKNOWN — you must author it) — filters: Team (`team_id`), Status (`state.id`), Creator (`creator.id`), Assignee (`assignee.id`), Priority (`priority`), Label (`label_ids`), Project (`project.id`), Project Milestone (`projectMilestone.id`)
- ▫ `lc_linear_new_issue_comment` (UNKNOWN — you must author it) — filters: Team (`team_id`), Issue (`issue.id`), Creator (`user.id`)
- ▫ `lc_linear_new_project` (UNKNOWN — you must author it) — filters: Team (`team_id`), Project Lead (`lead.id`), Status (`state`)
- ▫ `lc_linear_new_project_update` (UNKNOWN — you must author it) — filters: Team (`team_id`), Project (`project.id`), Creator (`user.id`)
- ▫ `lc_linear_updated_customer` (UNKNOWN — you must author it) — filters: Team (`team_id`)
- ▫ `lc_linear_updated_customer_need` (UNKNOWN — you must author it) — filters: Team (`team_id`), Customer (`customer.id`)
- ▫ `lc_linear_updated_issue` (UNKNOWN — you must author it) — filters: Team (`team_id`), Status (`state.id`), Creator (`creator.id`), Assignee (`assignee.id`), Priority (`priority`), Label (`label_ids`), Project (`project.id`), Project Milestone (`projectMilestone.id`)
- ▫ `lc_linear_updated_project_update` (UNKNOWN — you must author it) — filters: Team (`team_id`), Status (`health`), Lead (`project.lead.id`)

### Manus AI
- ▫ `lc_manus_new_task_created` (UNKNOWN — you must author it)
- ▫ `lc_manus_task_stopped` (UNKNOWN — you must author it)

### Monday.com
- ▫ `lc_monday_any_column_value_changed` (UNKNOWN — you must author it) — filters: Board Id (`board_id`), Column (`column_id`)
- ▫ `lc_monday_board_created` (UNKNOWN — you must author it)
- ▫ `lc_monday_item_moved_to_any_group` (UNKNOWN — you must author it) — filters: Board Id (`board_id`), Group Id (`group_id`)
- ▫ `lc_monday_new_item_created` (UNKNOWN — you must author it) — filters: Board Id (`board_id`)
- ▫ `lc_monday_new_subitem_created` (UNKNOWN — you must author it) — filters: Board Id (`board_id`)
- ▫ `lc_monday_new_update_in_board` (UNKNOWN — you must author it) — filters: Board Id (`board_id`)
- ▫ `lc_monday_user_added_to_board` (UNKNOWN — you must author it)

### Notion
- ▫ `notion_comment_added` (UNKNOWN — you must author it) — filters: Page ID (`id`)
- ▫ `notion_new_database_item` (UNKNOWN — you must author it) — filters: Database ID (`database_id`)
- ▫ `notion_page_updated` (UNKNOWN — you must author it) — filters: Page ID (`id`)
- ▫ `notion_updated_database_item` (UNKNOWN — you must author it) — filters: Database ID (`database_id`)

### Survey Monkey
- ▫ `survey_monkey_it_response_completed` (UNKNOWN — you must author it) — filters: Survey Id (`survey_id`)

### Todoist
- ▫ `lc_todoist_new_project` (UNKNOWN — you must author it)
- ▫ `lc_todoist_new_task` (UNKNOWN — you must author it) — filters: Project (`project_id`)
- ▫ `lc_todoist_task_completed` (UNKNOWN — you must author it) — filters: Project (`project_id`)

### Typeform
- ▫ `typeform_new_entry` (UNKNOWN — you must author it) — filters: Form (`form_id`), Submission Type (`event_type`)
