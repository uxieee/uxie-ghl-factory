...
### events
- ✅ `call_status` (highlevel) — filters: Call status (`call_status`), Custom disposition (`custom_disposition`), Call direction (`message.direction`), In workflow (`workflow.id`). Condition rows on `custom_disposition` / `call_status` are `operator: "contains-any"` with an ARRAY value and `type: "multiselect"` — the drawer's own shape. Dispositions are matched BY NAME, so the names must exist in Settings → Phone → Call dispositions or the trigger can never fire.
