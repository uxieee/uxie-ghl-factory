...
(`settings.senderAddress.from_number: "Main Line"` → its E.164), and trigger-filter values
...
| `WAIT_APPOINTMENT_CONDITION` | `appointmentCondition` outside `skip \| next \| specific-step \| exit` | The value `appointment` reads naturally and is wrong — it is the PAST-TIME behaviour, not which appointment. It SAVES and then fails the publish validator with "invalid value"; 55 legacy steps carry it |
| `ENTRY_NOT_FIRST` | The entry step is not `templates[0]` | **The runtime enters at array index 0**, not at the parentKey-less step. A root wired correctly but appended to the end of the array never executes, while the builder draws it as the head — proven live by runtime logs. Hatch: `allowEntryNotFirst` |
