...
| **opportunity lost reasons** | `list_account_entities` → `lostReasons` (internal), or `GET /opportunities/lost-reason?locationId=` — note the **SINGULAR** path; the plural 404s because the router reads it as an opportunity id. An `update_opportunity` with `status:'lost'` needs `lostReasonId`, and the engine resolves `lostReason` by NAME |
| **call dispositions** | `list_account_entities` → `callDispositions`, or `GET /phone-system/call-dispositions?locationId=`. A `call_status` trigger matches these **by name**, so a name absent from Settings can never fire — the compiler warns `TRIGGER_DISPOSITION_UNKNOWN`. Creating one needs `locationId` in the **QUERY** (403 in the body) |
