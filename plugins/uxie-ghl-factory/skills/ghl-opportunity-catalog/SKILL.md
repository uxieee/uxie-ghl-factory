...
description: The OPPORTUNITY lens for the whole-account GHL audit: rules for what is missing or under-used rather than broken, with the read that evidences each one. LOADED BY `/uxie-ghl-factory:audit` and its `surface-auditor` subagent — it is not a task a user asks for directly.
---
